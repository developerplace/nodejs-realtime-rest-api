...
class AlexaCli {
  ...
  installDependencies() {
    const result = execa.sync(
      'npm',
      ['install', '--prefix', this.getPath(...defaults.outDir), this.getPath(...defaults.outDir)],
  ...
  updateSkill(skillId, skillFileName = 'skill.json') {
    ...
    const result = execa.sync('ask', ['api', 'update-skill', '-s', skillId, '--stage', 'development', '--file', file], {
      cwd: this.rootPath,
      stdio: 'pipe',
    });
  ...
  updateModel(skillId, locale = 'de-DE', modelsPath = 'models') {
    ...
    const result = execa.sync(
      'ask',
      ['api', 'update-model', '-s', skillId, '--file', file, '--locale', locale, '--stage', 'development'],
  ...
  cloneSkill(skillId, locale = 'de-DE') {
    ...
    const result = execa.sync('ask', ['clone', '-s', skillId], { cwd: tmpPath, stdio: 'pipe' });
